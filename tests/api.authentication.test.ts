...
import Controller from '../src/web/controller/auth.controller';
...
describe('Authentication', () => {
    ...
    describe('Logging in', () => {
        it('Should log in', async () => {
            ...
            expect(response[0].login).to.equal(true);
        ...
        it('Should give an error with a wrong password', async () => {
            ...
            expect(response[0].login).to.equal(false);
    ...
    describe('My data', async () => {
        it ('Should give me my data (admin)', async () => {
            ...
            expect(response[0].user.role.role).to.be.a('string');
        });
